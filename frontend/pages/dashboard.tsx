    ...
    return (
        <div className="min-h-screen p-6 max-w-4xl mx-auto space-y-8">
            ...
            <section className="p-4 border rounded-2xl shadow">
                ...
                {formData.demographics ? (
                    <ul className="space-y-2">
                        <li><strong>Name:</strong> {formData.demographics.name}</li>
                        <li><strong>Date of Birth:</strong> {formData.demographics.dateOfBirth}</li>
                        <li><strong>Gender:</strong> {formData.demographics.gender}</li>
                        <li><strong>Race:</strong> {formData.demographics.race}</li>
                    </ul>
                ) : (
            ...
            <section className="p-4 border rounded-2xl shadow">
                ...
                {formData.employment ? (
                    <ul className="space-y-2">
                        <li><strong>Employer:</strong> {formData.employment.employer}</li>
                        <li><strong>Job Title:</strong> {formData.employment.jobTitle}</li>
                        <li><strong>Annual Income:</strong> ${formData.employment.annualIncome}</li>
                    </ul>
                ) : (
            ...
            <section className="p-4 border rounded-2xl shadow">
                ...
                {Object.keys(formData.medicalHistory).length > 0 ? (
                    <ul className="space-y-2">
                        {Object.entries(formData.medicalHistory).map(([questionId, answer]) => (
                            <li key={questionId}><strong>Q{questionId}:</strong> {answer}</li>
                        ))}
                    </ul>
                ) : (
                    ...
                    <ul className="space-y-2">
                        {formData.dependents.map((dep, index) => (
                            <li key={index} className="p-2 border rounded-xl">
                                <strong>{dep.name}</strong> - {dep.relationship} (DOB: {dep.dateOfBirth})
                            </li>
                        ))}
                    </ul>
                ) : (
            ...
            <div className="flex justify-center">
                <button onClick={() => router.push("/questionnaire")} className="mt-6">Edit Information</button>
            </div>
        </div>
    );
}
